...
class MultiTradeWatcher {
  static start() {
    try {
      ScriptApp.newTrigger(CHECK_ALL).timeBased().everyMinutes(1).create()
      Log.info(`Started ${CHECK_ALL}`)
    } catch (e) {
      Log.error(e)
    }
  }
  ...
  static stop() {
    const trigger = ScriptApp.getProjectTriggers().find(t => t.getHandlerFunction() == CHECK_ALL);
    if (trigger) {
      try {
        ScriptApp.deleteTrigger(trigger);
        Log.info(`Stopped ${CHECK_ALL}`)
      } catch (e) {
        Log.error(e)
      }
    }
...
function init(_runtimeCtx) {
  _runtimeCtx[CHECK_ALL] = function () {
