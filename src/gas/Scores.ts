...
export class Scores {
  ...
  #getRecommended(): CoinScore[] {
    const selectivity = this.config.ScoreSelectivity
  ...
  #updateScores(prices: PriceHoldersMap, marketMove: MarketMove): void {
    ...
    const belowCutoff = (s: number, n: number): boolean => n > 0 && n <= Math.ceil(s * total)
    const aboveCutoff = (s: number, n: number): boolean =>
      n <= total && n > Math.floor((1 - s) * total)
  ...
  #convertValuesToPercentages(map: object): object {
    const result = {}
